...
cursor.execute("""
        SELECT DATE(time) as thedate ,CAST(count(status) AS INT) as errors,CAST(count(ip) AS INT) as hits
        FROM log
        WHERE  status =  '404 NOT FOUND'
        GROUP BY thedate
        HAVING ROUND( CAST(count(status) AS INT) / CAST(count(ip) AS INT)  * 100 ) > 1.00
